...
namespace dsian.TwinCAT.AdsViewer.CapParser.Lib.Cap.AdsCommands
{
    internal class AdsAddDeviceNotificationResponse : IPayload
    {
        ...
        internal AdsAddDeviceNotificationResponse(AmsHeader amsHeader, BinaryReader binaryReader)
        {
        ...
        {
            return $"{nameof(AdsAddDeviceNotificationResponse)}: Res={Result}, NotificationHandle={NotificationHandle}";
        }
        private void ParsePacketData()
        {
            Result = (AdsErrorCode)BitConverter.ToUInt32(_PacketData, 0);
            NotificationHandle = BitConverter.ToUInt32(_PacketData, 4);
        }
    }
}
